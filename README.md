...
- 交易日志页提供独立的模拟账户，可按最新价格模拟开仓、平仓并自动监控止损/止盈；资金、持仓和历史记录保存在浏览器 IndexedDB。
- 模拟账户自动展示可用资金、保证金占用、浮动盈亏和已平仓盈亏；模拟记录与实盘日志分开保存。
- 可编辑、删除或排除实盘记录；已平仓模拟交易会与实盘记录一起进入周复盘和画像分析。
...
- 模拟账户已平仓记录参与方向、持仓时间、盈亏和退出方式统计；由于模拟表单不采集开仓依据，不会虚构其入场理由。
