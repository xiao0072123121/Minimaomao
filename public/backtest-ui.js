(function () {
  ...
  function tradeRow(trade) {
    ...
    return `<tr><td>${formatTime(trade.openAt)}</td><td class="${trade.side === "long" ? "positive" : "negative"}">${trade.side === "long" ? "做多" : "做空"}</td><td>${area}</td><td>${formatPrice(trade.entryPrice)}</td><td>${rsiMode ? trade.stopLabel : formatPrice(trade.initialStop)}</td><td>${rsiMode ? `${trade.target1Label}<small>${trade.target1Hit ? `已执行 ${formatPrice(trade.target1ExitPrice)}` : "未执行"}</small>` : formatPrice(trade.target1)}</td><td>${rsiMode ? trade.target2Label : formatPrice(trade.target2)}</td><td class="${trade.pnl > 0 ? "positive" : trade.pnl < 0 ? "negative" : ""}">${result}<small>${outcome}</small></td><td>${formatDuration(trade.closeAt - trade.openAt)}</td><td>${trade.signal}<small>${rsiMode ? `平仓：${trade.exitReason}` : trade.rsiNote}</small></td></tr>`;
  }
